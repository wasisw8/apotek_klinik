...
use App\Http\Requests\Master\SimpanJenisObatRequest;
use Carbon\Carbon;
...
class MasterJenisObatController extends Controller
{
    public function index()
    {
        ...
        return view('master.jenisobat.index')->with($data);
    }

    public function load(Request $request)
    {
        $data = DB::table('master_jenisobat')
            ->get();
    ...
    public function getData(Request $request)
    {
        $term = trim($request->q);
        $formatted_tags = [];
        $query = DB::table('master_jenisobat')
            ->select('id', 'nama');
    ...
    public function save(SimpanJenisObatRequest $request)
    {
        $validated = $request->validated();
        // cek apakah sudah ada Distributor
        $cek = DB::table('master_jenisobat')->where([
                    'nama'      => $validated['nama'],
                ])->count();
        if($cek>0){
    ...
    public function update(SimpanJenisObatRequest $request)
    {
        $validated = $request->validated();
    ...
    public function delete(Request $request)
    {
        DB::beginTransaction();
        try {
            DB::table('master_jenisobat')->where(['id' => $request->id])->delete();
            DB::commit();
            return response()->json([
