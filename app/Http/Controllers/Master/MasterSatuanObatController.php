...
use App\Http\Requests\Master\SimpanSatuanObatRequest;
use Carbon\Carbon;
...
class MasterSatuanObatController extends Controller
{
    ...
    {
        $data = DB::table('master_satuanobat')
            ->get();
    ...
    public function getData(Request $request)
    {
        $term = trim($request->q);
        $formatted_tags = [];
        $query = DB::table('master_satuanobat')
            ->select('id', 'nama');
    ...
    public function save(SimpanSatuanObatRequest $request)
    {
        $validated = $request->validated();
        // cek apakah sudah ada Distributor
        $cek = DB::table('master_satuanobat')->where([
                    'nama'      => $validated['nama'],
                ])->count();
        if($cek>0){
    ...
    public function update(SimpanSatuanObatRequest $request)
    {
        $validated = $request->validated();
    ...
    public function delete(Request $request)
    {
        DB::beginTransaction();
        try {
            DB::table('master_satuanobat')->where(['id' => $request->id])->delete();
            DB::commit();
            return response()->json([
