...
class MasterKategoriObatController extends Controller
{
    ...
    {
        $data = DB::table('master_kategoriobat')
            ->get();
    ...
    public function getData(Request $request)
    {
        $term = trim($request->q);
        $formatted_tags = [];
        $query = DB::table('master_kategoriobat')
            ->select('id', 'nama');
    ...
    public function save(SimpanKategoriObatRequest $request)
    {
        ...
        $cek = DB::table('master_kategoriobat')->where([
                    'nama'      => $validated['nama'],
                ])->count();
        if($cek>0){
    ...
    public function delete(Request $request)
    {
        DB::beginTransaction();
        try {
            DB::table('master_kategoriobat')->where(['id' => $request->id])->delete();
            DB::commit();
            return response()->json([
