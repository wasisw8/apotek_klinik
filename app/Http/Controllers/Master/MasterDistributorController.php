...
use App\Http\Requests\Master\SimpanDistributorRequest;
use Carbon\Carbon;
...
class MasterDistributorController extends Controller
{
    public function index()
    {
        ...
        return view('master.distributor.index')->with($data);
    }

    public function load(Request $request)
    {
        $data = DB::table('master_distributor')
            ->get();
    ...
    public function getData(Request $request)
    {
        $term = trim($request->q);
        $formatted_tags = [];
        $query = DB::table('master_distributor')
            ->select('id', 'nama');
    ...
    public function save(SimpanDistributorRequest $request)
    {
        $validated = $request->validated();
        // cek apakah sudah ada Distributor
        $cek = DB::table('master_distributor')->where([
                    'nama'      => $validated['nama'],
    ...
    public function update(SimpanDistributorRequest $request)
    {
        $validated = $request->validated();
    ...
    public function delete(Request $request)
    {
        DB::beginTransaction();
        try {
            DB::table('master_distributor')->where(['id' => $request->id])->delete();
            DB::commit();
            return response()->json([
